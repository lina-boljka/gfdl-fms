...
character(len=64) :: teq_mode = 'hs', damp_mode = 'hs', strat_damp = 'constant' , til = 'hs'
logical :: no_forcing = .false.
logical :: conserve_energy = .true.
logical :: strat_sponge = .true., strat_vtx = .true.
logical :: til_tf = .false. !impose TIL or not (set to true to impose TIL)
logical :: til_weight = .false. ! taper off the TIL strength towards the pole
...
real :: vtx_edge = 50.0, vtx_width = 10.0, vtx_gamma = 2.0, tp_width = 6.0
...
real :: q0_lsp = 0.0, m_lsp = 1.0, p0_lsp = 800.0, pt_lsp = 200.0, lat0_lsp = 45.0, slat_lsp = 10.0 , q0_lsptil = 6.
real :: trflux = 1.e-5 ! surface flux for optional tracer
real :: trsink = -4.   ! damping time for tracer
real :: lambda = 0. ! for Sjoberg's TIL definition - see his thesis, eqn. 4.4
real :: p_sjoberg = 20000. ! for Sjoberg's TIL definition (p_pert) - see his thesis, eqn. 4.4
real :: dtstrat = 27.
real :: const1 = 50., const2 = 5., const3 = 220., const4 = 40. ! constants needed for tanh representtion of tropopause height
real :: lat_tap = 75. ! used if til_weight is true to taper off TIL strength
real :: exp_trop = -33. ! exponent in the troposphere for tIL - not used is TIL is false etc. ; I have tested -13., -18., -33. before... 
...
namelist /forcing_nml/  no_forcing, locked_heating, conserve_energy, &
  ...
  x0_arctic, y0_arctic,   q0_lsptil, lambda, til, p_sjoberg, dtstrat, & 
  const1, const2, const3, const4, til_tf , tp_width, til_weight, lat_tap, &
  exp_trop,
...
subroutine forcing ( is, ie, js, je, dt, Time, lon, lat, p_half, p_full, z_full, &
  ...
  if (locked_heating) then
    ...
    call thermal_damping ( lat,lon, sigma, p_full, z_full, t, tdt_damp, tdamp, teq, mask=mask )
  endif
...
subroutine forcing_init ( axes, is, ie, js, je, num_levels, Time )
  ...
  if ((trim(teq_mode) .ne. 'hs') .and. (trim(teq_mode) .ne. 'pk') .and. (trim(teq_mode) .ne. 'pkmod') .and. (trim(teq_mode) .ne. 'pktil')) then
...
subroutine thermal_damping ( lat,lon, sigma, p_full, z_full, t, tdt, tdamp, teq, mask)
...
real, dimension(size(t,1),size(t,2)) :: &
  ...
  lat_hfact, lat_vfact, w_vtx, w_tp, p_trop3,  &
  z_vortex, z_offset, z_norm, p_trop, p_norm, p_trop2, pbot1, ptop1, p_norm1 !, z_vortex1, z_offset1 ! intermediate containers
real, dimension(size(t,1)) :: &
  p_trop2_1D, p_trop_1D, p_trop3_1D !holds zonal mean p_tropopause
...
real :: p0, pb, pfact, pbot, ptop, p_trop2_y, p_trop_y 
real :: z_vortex_ref, p_trop_ref, t_surf_ref
real :: lat_ref_r, lat_vfact_ref, lat_hfact_ref
real :: vtx_edge_r, vtx_width_r, const3_r, tp_width_r, lat_tap_r
integer :: i, j, k, c, seconds, days, y !, dtstrat
...
tp_width_r    = tp_width*pi/180.0
vtx_edge_r    = vtx_edge*pi/180.0
const3_r      = const3*pi/180.
lat_tap_r     = lat_tap*pi/180.
...
if (til_tf) then
  ! get TIL stuff!!!
  p_trop = p0*( (t_strat - dtstrat) / (t_surf - delv*log(pfact)*lat_vfact) )**(1.0/kappa)
  !p_trop_ref = p0*( t_strat / (t_surf_ref - delv*log(pfact)*lat_vfact_ref) )**(1.0/kappa)
  !z_vortex1 = -H*log(p_trop/p0)
  !z_offset1 = (z_vortex1 + H*log(p_trop_ref/p0)) + (z_ozone - 20.0) ! includes offset to accomodate configurable standard atmosphere height
  p_trop2 = p0*( (t_strat - dtstrat) / (t(:,:,size(t,3)) - delv*log(p_logeval*100.0/p0)*lat_vfact) )**(1.0/kappa) ! make it time dependent; NB: temp(k=N) seems to be sfc temp, not temp(k=1)

  ! get zonal mean - so we can loop over latitudes below
  p_trop_1D = sum(p_trop,1)/size(p_trop,1)
  p_trop2_1D = sum(p_trop2,1)/size(p_trop2,1)

  p_trop3 = const1 * tanh(const2*(lat-const3_r)) + const4
  w_tp = 0.5*(1.0 + tanh((lat - abs(const3_r))/tp_width_r))
  p_trop3_1D = 100 * sum(p_trop3,1)/size(p_trop3,1) ! multiply by 100 to get Pa; above tanh computation is for hPa!!!

  if (til_weight) then
    where (lat > (const3_r + lat_tap_r)/2.0)
      w_tp = 0.5*(1.0 - tanh((lat - abs(lat_tap_r))/tp_width_r))
    endwhere
  endif
  !print *, size(p_trop_1D)
...
do k=1,size(t,3)
  ...
  if (til_tf) then
    ! if we want to impose TIL here are some options how to do it..... 
    if (trim(til) == 'sjoberg') then
      p_norm1 = p_full(:,:,k)/p_sjoberg ! default p_sjoberg is 20000Pa
      where (p_full(:,:,k) <= p_sjoberg)
        teq(:,:,k) = teq(:,:,k) + (lambda * w_tp * p_norm1**(13./3.))
      endwhere

    

    else if (trim(til) == 'sjobergmod') then
      do y=1,size(t,2)
	p_trop_y = p_trop_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop_y ! use Luke's p_trop (i.e. pressure of 200K at each lat - but also seems constant)
        where (p_full(:,y,k) <= p_trop_y) ! don't allow tropopause stuff to start further down than 400mb.
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * p_norm1(:,y)**(13./3.))
        endwhere
      enddo
    else if (trim(til) == 'sjobergmod_t') then
      do y=1,size(t,2)
	p_trop2_y = p_trop2_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop2_y ! use Luke's p_trop (i.e. pressure of 200K at each lat - but also seems constant); time dependent
        where (p_full(:,y,k) <= p_trop2_y) ! don't allow tropopause stuff to start further down than 400mb.
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * p_norm1(:,y)**(13./3.))
        endwhere
      enddo

    else if (trim(til) == 'sjoberg_strat') then
      do y=1,size(t,2)
        p_trop_y = p_trop3_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop_y ! use tanh p_trop
        where (p_full(:,y,k) <= p_trop_y) ! don't allow tropopause stuff to start further down than 400mb.
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * p_norm1(:,y)**(13./3.))
        endwhere
      enddo

    else if (trim(til) == 'sjoberg_trop') then
      do y=1,size(t,2)
        p_trop_y = p_trop3_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop_y ! use tanh p_trop
        where (p_full(:,y,k) >= p_trop_y) ! don't allow tropopause stuff to start further down than 400mb.
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * p_norm1(:,y)**(exp_trop/3.)) ! used to be -33; now -18 (slower tapering off!)
        endwhere
      enddo

    else if (trim(til) == 'sjoberg_both') then
      do y=1,size(t,2)
        p_trop_y = p_trop3_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop_y ! use tanh p_trop
        where (p_full(:,y,k) <= p_trop_y) ! don't allow tropopause stuff to start further down than 400mb.
    ...
    else if (trim(til) == 'gauss_both') then
      do y=1,size(t,2)
        p_trop_y = p_trop3_1D(y)
        p_norm1(:,y) = p_full(:,y,k)/p_trop_y ! use tanh p_trop
        where (p_full(:,y,k) <= p_trop_y .and. p_full(:,y,k) >= p_trop_y-15000.) ! don't allow tropopause stuff to start further down than 400mb.
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * exp((-1)*((p_norm1(:,y)-1.)**2)/0.03)) ! 1/0.03 = 33.33333
        elsewhere (p_full(:,y,k) > p_trop_y .and. p_full(:,y,k) <= p_trop_y+10000.) ! at p_trop-150. ~50hPa above & p_trop+100.~300hPa here the precision drops to 1e-7 in gaussian distribution
          teq(:,y,k) = teq(:,y,k) + (lambda * w_tp(:,y) * exp((-1)*((p_norm1(:,y)-1.)**2)/0.01)) ! 1/0.01 = 100.
        endwhere
      enddo



    else if (trim(til) == 'lsp') then
      ! use lsp function stuff
      lat0_lsp = lat0_lsp * pi/180.0 ! latitude at which localised heating is centred
      slat_lsp = slat_lsp * pi/180.0 ! 0.175 radians default, presumably related to the width of heating in latitude (standard deviation in Gaussian distribution)
      pbot = p0_lsp*100.0 -10000. ! make it 100mb below assumed tropopuase level since max in lsp is above bottom level
      ptop = pt_lsp*100.0 -10000. ! make it 200mb above when defining pt!!
      where ((p_full(:,:,k) <= pbot) .and. (p_full(:,:,k) >= ptop))
        tdt(:,:,k,1) = tdt(:,:,k,1) + &
          q0_lsptil * sin(m_lsp*lon(:,:))*exp(-0.5*((lat - lat0_lsp)/slat_lsp)**2)*sin(pi*log(p_full(:,:,k)/pbot)/log(ptop/pbot)) &
            / seconds_per_day
      endwhere 

    else if (trim(til) == 'lspmod') then
      ! call lsp function stuff but make it latitudinally dependent
      lat0_lsp = lat0_lsp * pi/180.0 ! latitude at which localised heating is centred
      slat_lsp = slat_lsp * pi/180.0 ! 0.175 radians default, presumably related to the width of heating in latitude (standard deviation in Gaussian distribution)
      pbot1 = p_trop-10000. ! make it 100mb below since max in lsp is above bottom level
      ptop1 = (pbot1+20000.) ! make it 200mb above
  
      where ((p_full(:,:,k) <= min(maxval(pbot1),40000.)) .and. (p_full(:,:,k) >= minval(ptop1)))
        tdt(:,:,k,1) = tdt(:,:,k,1) + &
          q0_lsptil * sin(m_lsp*lon(:,:))*exp(-0.5*((lat - lat0_lsp)/slat_lsp)**2)*sin(pi*log(p_full(:,:,k)/pbot1)/log(ptop1/pbot1)) &
            / seconds_per_day
      endwhere 

    else if (trim(til) == 'lspmod_t') then
      ! call lsp function stuff but make it latitudinally dependent and time dependent
      lat0_lsp = lat0_lsp * pi/180.0 ! latitude at which localised heating is centred
      slat_lsp = slat_lsp * pi/180.0 ! 0.175 radians default, presumably related to the width of heating in latitude (standard deviation in Gaussian distribution)
      pbot1 = p_trop2-10000. ! make it 100mb below since max in lsp is above bottom level
      ptop1 = (pbot1+20000.) ! make it 200mb above pbot
